...
export async function getAllProducts(
  ...
  try {
    ...
    // FIXED: Simplified image join logic
    // Instead of complex conditional logic, let's get all images and prioritize correctly
    const imagesJoin = db
      .select({
        productId: productImages.productId,
        url: productImages.url,
    ...
    switch (filters.sort) {
      case "price_asc":
        primaryOrder = asc(sql`min(${variantJoin.price})`);
        break;
      case "price_desc":
        primaryOrder = desc(sql`max(${variantJoin.price})`);
        break;
      case "newest":
    ...
    console.log("=== DEBUG: Main query returned ===", rows.length, "products");
...
// Also create a simple query to check what images exist
